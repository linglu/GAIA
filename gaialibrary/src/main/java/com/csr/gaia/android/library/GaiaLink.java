   ...
   // End of public fields
    ...
    private void connectBluetooth(String address) throws IOException
    {
        ...
        if (!getBluetoothAvailable())
            ...
            DebugLog.d(DebugLog.TAG, "GaiaLink:connectBluetooth " + "connect BT " + address);
            mBTDevice = mBTAdapter.getRemoteDevice(upper_address);
            
            switch (mTransport) {

            case BT_GAIA:
                DebugLog.d(DebugLog.TAG, "GaiaLink:connectBluetooth " + "BT_GAIA");
                mBTSocket = createSocket(GAIA_UUID);
                break;
                
            case BT_SPP:
                DebugLog.d(DebugLog.TAG, "GaiaLink:connectBluetooth " + "BT_SPP");
...
//            }
    ...
    {
        DebugLog.d(DebugLog.TAG, "GaiaLink:sendCommand " + "payload_length: " + payload_length);
    ...
    {
        DebugLog.d(DebugLog.TAG, "GaiaLink:sendCommand " + "payload : " + Utils.printHexString(payload));
    ...
    {
        DebugLog.d(DebugLog.TAG, "GaiaLink:sendCommand "
                                + "vendor_id: " + Utils.intToHex(vendor_id)
                                + " command_id : " + Utils.intToHex(command_id)
                                + " length : " + param.length);
    ...
    */
    ...
    {        
        ...
        private void runSppReader() {
            ...
            {
                DebugLog.d(DebugLog.TAG, "Reader:runSppReader " + " connected");
                mReceiveHandler.obtainMessage(Message.CONNECTED.ordinal(), mBTDevice.getAddress()).sendToTarget();
                mIsConnected = true;
                going = true;
            }
...
//                        if (mVerbose) Log.d(TAG, "expect " + expected);
                        DebugLog.d(DebugLog.TAG, "Reader:scanStream " + "expect " + expected);
                    }
...
//                        if (mVerbose) Log.d(TAG, "got " + expected);
                        DebugLog.d(DebugLog.TAG, "Reader:scanStream " + "got " + expected);
                        
                        if (mReceiveHandler == null)
                        {
//                            if (mDebug) Log.e(TAG, "No receiver");
                            DebugLog.d(DebugLog.TAG, "Reader:scanStream " + "No receiver");
                        }
                        ...
                        else
                        {
                            ...
                            {
                                DebugLog.d(DebugLog.TAG, "Reader:scanStream " + "start");
                                mReceiveHandler.obtainMessage(Message.CONNECTED.ordinal(), mBTDevice.getAddress()).sendToTarget();
                                mIsConnected = true;
                            }
